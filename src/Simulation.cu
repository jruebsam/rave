...
#define LAPLACE(v) (inv_dx_quad * (v[East] - 2 * v[Point] + v[West]) + inv_dz_quad * (v[North] - 2 * v[Point] + v[South]))

#define ADVECT(v) ((axp * (v[Point] - v[West]) / dx + axm * (v[East] - v[Point]) / dx + azp * (v[Point] - v[South]) / dz + azm * (v[North] - v[Point]) / dz))
